...
func _create_upgrade_buttons():
	...
	var kitchen_upgrades = upgrade_manager.get_upgrades_by_category(
		UpgradeEnums.UpgradeCategory.KITCHEN
	)
...
func _create_worker_assignment_buttons():
	...
	for worker in hired_workers:
		...
		assignment_button.pressed.connect(
			_on_worker_assignment_button_pressed.bind(worker["worker_id"])
		)
...
func _get_worker_assignment_text(worker: Dictionary) -> String:
	...
	var assignment_name = (
		"Kitchen" if worker["assignment"] == WorkerDefinition.WorkerAssignment.KITCHEN else "Office"
	)
...
func _on_worker_assignment_button_pressed(worker_id: String):
	...
	var new_assignment = (
		...
		if worker["assignment"] == WorkerDefinition.WorkerAssignment.KITCHEN
		else WorkerDefinition.WorkerAssignment.KITCHEN
	)
