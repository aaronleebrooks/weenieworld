...
func _on_purchase_timer_timeout():
	...
	if hot_dog_manager.hot_dogs_inventory >= purchase_amount:
		if hot_dog_manager.sell_hot_dogs(purchase_amount):
			...
			print("CustomerManager: Customer purchased %d hot dogs for %d currency" % [purchase_amount, earned_value])
		else:
			print("CustomerManager: Failed to sell hot dogs to customer")
	else:
		print("CustomerManager: Not enough hot dogs for customer (have %d, need %d)" % [hot_dog_manager.hot_dogs_inventory, purchase_amount])
