...
func get_next_hire_cost() -> int:
	...
	match worker_count:
		...
		1: return 100
		2: return 1000
		_: return 1000 + (worker_count - 2) * 500  # Scaling cost for additional workers

func hire_worker(assignment: WorkerDefinition.WorkerAssignment = WorkerDefinition.WorkerAssignment.KITCHEN) -> bool:
	...
	if event_log_manager:
		event_log_manager.add_worker_event(
			"Worker Hired", 
			"Hired %s for %d currency" % [worker_data["display_name"], hire_cost]
		)
...
func assign_worker(worker_id: String, assignment: WorkerDefinition.WorkerAssignment) -> bool:
	...
	var assignment_name = "Kitchen" if assignment == WorkerDefinition.WorkerAssignment.KITCHEN else "Office"
