...
func get_save_data() -> Dictionary:
	...
	return {"upgrades": {"levels": upgrade_levels, "total_purchased": total_upgrades_purchased}}
...
func purchase_upgrade(upgrade_id: String) -> bool:
	...
	print(
		(
			"UpgradeManager: Purchased %s (level %d) for %d currency"
			% [upgrade.display_name, upgrade_levels[upgrade_id], cost]
		)
	)
	return true
...
func _apply_upgrade_effects(upgrade_id: String):
	...
	match upgrade.effect_type:
		...
		UpgradeEnums.EffectType.CUSTOMER_PURCHASE_RATE:
			...
			emit_signal(
				"upgrade_effect_applied", upgrade_id, "customer_purchase_rate", total_effect
			)
