...
var event_templates: Dictionary = {
	"office_purchased": {
		...
		"description": "You've purchased an office! Workers can now be hired and assigned to stations.",
		"icon": "🏢"
	},
	"worker_hired": {
...
func add_event(event_type: EventType, title: String, description: String, icon: String = "📝", data: Dictionary = {}):
...
func add_purchase_event(item_name: String, cost: int, item_type: String = "item"):
	...
	add_event(EventType.PURCHASE, title, description, icon, {
		"item_name": item_name,
		"cost": cost,
		"item_type": item_type
	})
...
func add_save_event(save_type: String = "manual"):
	...
	add_event(EventType.SAVE, title, description, icon, {
		"save_type": save_type
	})
